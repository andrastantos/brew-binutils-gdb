...
static const char *
brew_register_name (struct gdbarch *gdbarch, int reg_nr)
{
  if (reg_nr < 0)
    return NULL;
  if (reg_nr >= sizeof(brew_register_names)/sizeof(brew_register_names[0]))
    return NULL;
  return brew_register_names[reg_nr];
}
...
static void
brew_store_return_value (struct type *type, struct regcache *regcache,
                         const gdb_byte *valbuf)
{
  struct gdbarch *gdbarch = regcache->arch ();
  enum bfd_endian byte_order = gdbarch_byte_order (gdbarch);
  gdb_assert(byte_order == BFD_ENDIAN_BIG);
  CORE_ADDR regval;
  ...
  for (int ofs = 0; ofs < len; ofs+=4)
    {
      ...
      regval = extract_unsigned_integer(valbuf+ofs, (len-ofs) > 4 ? 4 : len, byte_order);
...
static CORE_ADDR
brew_analyze_prologue (CORE_ADDR start_addr, CORE_ADDR end_addr,
...
static CORE_ADDR
brew_skip_prologue (struct gdbarch *gdbarch, CORE_ADDR pc)
{
...
/* Read an unsigned integer from the inferior, and adjust
   endianness.  */
static ULONGEST
brew_process_readu (CORE_ADDR addr, gdb_byte *buf,
...
brew_software_single_step (struct regcache *regcache)
{
  struct gdbarch *gdbarch = regcache->arch ();
  CORE_ADDR addr;
  gdb_byte buf[4];
  ...
  inst = (uint16_t) brew_process_readu (addr, buf, 2, byte_order);
  ...
  if (inst & (1 << 15))
  ...
  else
    {
      ...
      switch (opcode)
        ...
        case 0x03: /* jsra */
        case 0x1a: /* jmpa */
          next_pcs.push_back (brew_process_readu (addr + 2, buf, 4,
                                                   byte_order));
          break;
...
static void
brew_extract_return_value (struct type *type, struct regcache *regcache,
                            gdb_byte *dst)
{
  struct gdbarch *gdbarch = regcache->arch ();
  enum bfd_endian byte_order = gdbarch_byte_order (gdbarch);
...
brew_return_value (struct gdbarch *gdbarch, struct value *function,
...
/* Allocate and initialize a brew_frame_cache object.  */

static struct brew_frame_cache *
brew_alloc_frame_cache (void)
{
  struct brew_frame_cache *cache;
  int i;

  cache = FRAME_OBSTACK_ZALLOC (struct brew_frame_cache);
...
/* Populate a brew_frame_cache object for this_frame.  */

static struct brew_frame_cache *
brew_frame_cache (struct frame_info *this_frame, void **this_cache)
{
  struct brew_frame_cache *cache;
  ...
  if (*this_cache)
    return (struct brew_frame_cache *) *this_cache;
  ...
  if (cache->pc)
    {
      ...
      brew_analyze_prologue (cache->pc, current_pc, cache, gdbarch);
    }
...
static void
brew_frame_this_id (struct frame_info *this_frame,
                    void **this_prologue_cache, struct frame_id *this_id)
{
  struct brew_frame_cache *cache = brew_frame_cache (this_frame,
                                                   this_prologue_cache);
...
static struct value *
brew_frame_prev_register (struct frame_info *this_frame,
...
static CORE_ADDR
brew_frame_base_address (struct frame_info *this_frame, void **this_cache)
{
  struct brew_frame_cache *cache = brew_frame_cache (this_frame,
                                                       this_cache);
...
static int
brew_process_record (struct gdbarch *gdbarch, struct regcache *regcache,
                      CORE_ADDR addr)
{
  gdb_byte buf[4];
  uint16_t inst;
  ...
  if (inst & (1 << 15))
      ...
      if (inst & (1 << 14))
      ...
      else
        {
          ...
          switch (opcode)
            ...
            case 0x03: /* ssr */
              {
                /* Do nothing until GDB learns about brew's special
                   registers.  */
              }
              break;
            default:
  ...
  else
    {
      ...
      switch (opcode)
        ...
        case 0x0e: /* cmp */
          {
//            if (record_full_arch_list_add_reg (regcache, BREW_CC_REGNUM))
              return -1;
          }
          break;
        case 0x0f: /* nop */
        ...
        case 0x1f: /* sta.b */
          {
            tmpu32 = brew_process_readu (addr+2, buf, 4, byte_order);
            if (record_full_arch_list_add_mem (tmpu32, 1))
              return -1;
          }
          break;
...
brew_gdbarch_init (struct gdbarch_info info, struct gdbarch_list *arches)
{
  struct gdbarch *gdbarch;
  ...
  set_gdbarch_breakpoint_kind_from_pc (gdbarch,
                                       brew_breakpoint::kind_from_pc);
  set_gdbarch_sw_breakpoint_from_kind (gdbarch,
                                       brew_breakpoint::bp_from_kind);
