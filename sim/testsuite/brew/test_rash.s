...
.include "framework/framework.s"
  ...
  .endr
  \RD <- \RB >> \RA
  # We have to sign-extend to the upper 32-bits before the shift, which is painful in bignums...
  .set EXP_VAL, ( ((-(IN_R\RB >> 31)) << 32 | IN_R\RB) >> (IN_R\RA & 31) ) & 0xffffffff
  TEST \RD, EXP_VAL
  .endm
  ...
  .endr
  \RD <- \RB >> (\IMM)
  .set EXP_VAL, ( ((-(IN_R\RB >> 31)) << 32 | IN_R\RB) >> (\IMM & 31) ) & 0xffffffff
  TEST \RD, EXP_VAL
  .endm
  ...
  .endr
  \RD <- (\IMM) >> \RA
  .set EXP_VAL, ( ((-(\IMM >> 31)) << 32 | \IMM) >> (IN_R\RA & 31) ) & 0xffffffff
  TEST \RD, EXP_VAL
  .endm
