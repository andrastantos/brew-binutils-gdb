...
static reloc_howto_type brew_elf_howto_table [] =
{
  ...
  /* TODO: add a PC relative relocation (last entry) */
  /* A 32 bit absolute relocation.  */
  HOWTO (R_BREW_32,		/* type */
	 0,			/* rightshift */
...
brew_reloc_type_lookup (bfd *abfd ATTRIBUTE_UNUSED,
			 bfd_reloc_code_real_type code)
{
  unsigned int i;

  for (i = sizeof (brew_reloc_map) / sizeof (brew_reloc_map[0]);
       i--;)
...
static reloc_howto_type *
brew_reloc_name_lookup (bfd *abfd ATTRIBUTE_UNUSED, const char *r_name)
{
  unsigned int i;

  for (i = 0;
       i < sizeof (brew_elf_howto_table) / sizeof (brew_elf_howto_table[0]);
       i++)
    if (brew_elf_howto_table[i].name != NULL
...
static bool
brew_info_to_howto_rela (bfd *abfd,
			  arelent *cache_ptr,
...
brew_final_link_relocate (reloc_howto_type *howto,
			   bfd *input_bfd,
...
static int
brew_elf_relocate_section (bfd *output_bfd,
			    ...
			    asection **local_sections)
{
  ...
  for (rel = relocs; rel < relend; rel ++)
	...
	continue;
      ...
      r = brew_final_link_relocate (howto, input_bfd, input_section,
...
static asection *
brew_elf_gc_mark_hook (asection *sec,
			struct bfd_link_info *info,
			Elf_Internal_Rela *rel,
			struct elf_link_hash_entry *h,
			Elf_Internal_Sym *sym)
...
static bool
brew_elf_check_relocs (bfd *abfd,
			struct bfd_link_info *info,
			asection *sec,
