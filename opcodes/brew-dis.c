...
int
print_insn_brew (bfd_vma addr, struct disassemble_info * info)
{
  int length = 2;
  int status;
  stream = info->stream;
  ...
  if ((iword & (1<<15)) == 0)
    {
      ...
      opcode = &brew_form1_opc_info[iword >> 8];
      switch (opcode->itype)
	{
	case BREW_F1_NARG:
  ...
  else if ((iword & (1<<14)) == 0)
    {
      ...
      opcode = &brew_form2_opc_info[(iword >> 12) & 3];
      switch (opcode->itype)
	{
	case BREW_F2_A8V:
	  fpr (stream, "%s\t%s, 0x%x",
  ...
  else
    {
      ...
      opcode = &brew_form3_opc_info[(iword >> 10) & 15];
      switch (opcode->itype)
	{
	case BREW_F3_PCREL:
	  fpr (stream, "%s\t", opcode->name);
